program main  !! Test get_communicator language extension
  ...
  associate(initial_image=>this_image(), initial_num_images=>num_images(), new_team=>mod(this_image()+1,num_teams)+1)
    form team(new_team,league) !! create mapping
    change team(league) !! join child team
      ...
      associate(my_team=>team_number())
        call assert(my_team==new_team,"assigned team matches chosen team")
