...
without the use of \glspl{coarray}.  These include the ability to declare teams of images that communicate only within their teams by default.

In the context of \gls{caf}, we propose a working definition of ``embarrassingly parallel'' as denoting the class of parallel
...
concurrent execution of ensemble members being handled by separate teams.  This paper presents such a use case in the context of a terrestrial
hydrological model developed at \gls{ncar}: \gls{wrf-hydro}.
...
language extension that assists with incrementally introducing
\gls{caf} into existing \gls{mpi} applications when using teams.
...
%% JLM: The above sections listed are actually in a different order below. I kind of like discussing the application first, personally.

%% JLM: Do we want ro reorder the figures if we stick with WRF-Hydro first in the methodology?

\subsection{Objectives}\label{sec:objectives}
The immediate objectives of this work are to:
\begin{enumerate}
  \item Run WRF-Hydro ensembles within a single fortran executable.
  \item Allow for migration of WRF-Hydro model code from MPI to CAF in the future.
\end{enumerate}

These immediate goals lay the foundation for exploring code
restructuring aimed at amortizing common model initialization across
all available computational resources rather than repeating common
aspects of the initialization in each ensemble member using only that
member's fraction of the hardware resources available to
each. 
...
Figure ? can greatly reduce the amount of labor involved in designing
workflows and can open up new
...
initialization just once the full number of images prior to forming teams. Communicating the shared initialization
to each team should provide particular computational savings for
ensemble model runs where initialization comprises a large fraction of
the total run time. 



Going from a deterministic run to an ensemble run.
